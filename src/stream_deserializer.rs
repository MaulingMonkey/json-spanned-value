...
use crate::{Buffer, Reader, Settings, Shared, SharedStack};
...
pub struct StreamDeserializer<'de, B: Buffer, T: Deserialize<'de>> {
    base:   BaseStreamDeserializer<'de, IoRead<Reader<B>>, T>,
    shared: Rc<Shared>,
}

impl<'de, B: Buffer, T: Deserialize<'de>> StreamDeserializer<'de, B, T> {
...
impl<'de, B: Buffer, T: Deserialize<'de>> Iterator for StreamDeserializer<'de, B, T> {
    type Item = Result<T>;
...
impl<'de, B: Buffer, T: Deserialize<'de>> FusedIterator for StreamDeserializer<'de, B, T> {}
